...
FAO_qua <- FAO_raw %>% 
  ...
  summarize(Metric = sum(QUANTITY, na.rm = TRUE)) %>% 
  as_tsibble(index = YEAR, key = c("Genus", "Country", "Region", "SubRegion"))

FAO_val <- FAO_raw %>% 
  select(-QUANTITY) %>% 
  group_by(Genus,Country, YEAR, Region, SubRegion) %>% 
  summarize(Metric = sum(VALUE, na.rm = TRUE)) %>% 
  as_tsibble(index = YEAR, key = c("Genus", "Country", "Region", "SubRegion"))
...
fc %>% 
  filter(is_aggregated(Region), is_aggregated(Genus)) %>% 
  autoplot(
    sw_agg %>% filter(YEAR>=2009),
    level = NULL) +
